...
def test():
    ...
    image_encoder = ConvNeXt(depths=[3, 3, 27, 3], dims=[96, 192, 384, 768]).to(device)
    number_encoder = linear_encoder().to(device)
    decoder = linear_decoder(in_channel=1094, out_channel=6, hidden_channel=256).to(
        device
    )

    weightFile = os.path.join("checkPoints/plant_regression_v0.4", savemodelName + ".tar")
    ...
    predict_df.to_csv(os.path.join(data_root, "predict_100.csv"), index=False)
